...
def rot13(text):
    """Encrypt or decrypt the input text using ROT13."""
    result = ""
    for char in text:
        if char.isalpha():
...
button_7 = Button(
    ...
    borderwidth=0,
    highlightthickness=0,
    command=lambda: encrypt_button_command(entry_1, entry_3),
    relief="flat",
    font=("Arial", 18)
)
button_7.place(
...
def encrypt_button_command(entry_1, entry_3):
    ...
    if message == "":
        ...
        entry_3.delete(0, 'end')  # Clear previous content
        entry_3.insert(0, rot13(message))
...
button_8 = Button(
    ...
    borderwidth=0,
    highlightthickness=0,
    command=lambda: decrypt_button_command(entry_4, entry_6),
    relief="flat",
    font=("Arial", 18)
)
button_8.place(
...
def decrypt_button_command(entry_4, entry_6):
    cipher_text = entry_4.get().replace(' ', '')
    
    if cipher_text == "":
        messagebox.showerror("Error", "Please enter a cipher text.")
    elif not cipher_text.isalpha():
        messagebox.showerror("Error", "Cipher text must be a string.")
    else:
        entry_6.delete(0, 'end')  # Clear previous content
        entry_6.insert(0, rot13(cipher_text))

button_1.configure(command=lambda: update_entry_with_file(entry_1))
button_7.configure(command=lambda: encrypt_button_command(entry_1, entry_3))
...
button_8.configure(command=lambda: decrypt_button_command(entry_4, entry_6))
button_4.configure(command=lambda: save_to_file(entry_6.get(), 'decrypted_message.txt'))

button_6.configure(command=lambda: window.destroy())
